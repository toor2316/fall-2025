...
"""
    estimate_factor_analysis_regression(df::DataFrame)
...
==================================================================================#
 include("lgwt.jl") 
  # For Gauss-Legendre quadrature nodes and weights
"""
...
result = optimize(td, start_vals, Newton(linesearch = LineSearches.BackTracking()),
                  Optim.Options(g_tol = 1e-5, iterations = 100_000, show_trace = true, show_every = 10))
...
function main()
    ...
    println(df[1:5, :])  # Display first 5 rows
    println(describe(df))  
...
# Question 2
    ...
    println("Multicollinearity is a problem: It does not make sense to include all ASVAB scores simultaneously.") 

    # Question 4
    println("\n" * "="^80)
    println("Question 4: PCA Regression")
    println("="^80)
    # Call estimate_pca_regression() and display results
    df = generate_pca!(df)
   println(describe(df)) 
 ...
 svals= vcat( 
    ...
    Xfac\asvabs[:,5], 
    Xfac\asvabs[:,6],
 # for beta: regress wage on X 
  X\y,
 # for alpha: use std dev of asvabs and wage 
 rand(7), 
 # for sigma: start with siample std devs or 0.5 
  0.5*ones(7)
    ) 

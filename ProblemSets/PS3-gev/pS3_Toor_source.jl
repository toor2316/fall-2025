...
function mlogit_with_Z(theta, X, Z, y)
    ...
    # Reshaping alpha into K x (J-1) matrix, adding zeros for normalized choice
    bigAlpha = [reshape(alpha, K, J-1) zeros(K)]
    
    # Initialize probability matrix,computing probabilities and log-likelihood 
...
function nested_logit_with_Z(theta, X, Z, y, nesting_structure)
    ...
    #Calculating probabilities and log-likelihood for nested logit
    # Computing linear indices
    for j = 1:J

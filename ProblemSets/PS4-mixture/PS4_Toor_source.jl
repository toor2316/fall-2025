...
function mlogit_with_Z(theta, X, Z, y)
    ...
    # computing numerator for each choice j
    for j = 1:J
...
function practice_quadrature()
    ...
    # Verifying integral of density equals 1
...
function variance_quadrature()
   ...
   variance_10pts = sum(weights10 .* (nodes10.^2) .* pdf.(d, nodes10)) 
    ...
    println("More grid points give a better approximation")
end
...
function mixed_logit_quad(theta, X, Z, y)
    ...
    # loop through grip points to do summation via loops 
...
function allwrap()
    ...
    # Uncomment when ready to estimate
    ...
    println("same interpretation as in PS3; this time it's large and positive with t-stat of >100")
